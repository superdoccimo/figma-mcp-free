...
async function assertMcpStarts(launcher, cwd) {
  await new Promise((resolvePromise, reject) => {
    ...
    child.once("exit", (code, signal) => {
      ...
      finish(new Error(`MCP process exited before the smoke window with ${detail}.\n${stderr}`));
    });
    ...
    child.once("close", () => {
      if (!finished && shuttingDown) finish();
    });
  });
}
...
export async function smokeDistribution() {
  ...
  try {
    ...
    run(npmCommand, ["install", "--ignore-scripts", "--no-audit", "--no-fund", packed.tarballPath], { cwd: installRoot });
    ...
    runInstalled(launcher, ["plugin", "create-manifest", "123456789", "--port", "49152", "--out-dir", pluginDir, "--json"], { cwd: installRoot });
