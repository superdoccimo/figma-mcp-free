...
function addConnectionOptions(command: Command): Command {
  return command
    .option("--url <url>", "bridge origin (default FIGMA_PLUGIN_BRIDGE_URL or http://127.0.0.1:3845)")
...
program.command("serve")
  ...
  .option("--max-nodes <n>", "maximum document nodes per captured selection", parsePositiveInteger, 10000)
  .option("--max-depth <n>", "maximum REST document child depth", parseInteger, 64)
  .option("--request-timeout <ms>", "server request timeout", parsePositiveInteger, 10000)
  .option("--json", "print startup information as JSON; output contains the sensitive token")
