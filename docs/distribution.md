...
The public distribution is intentionally a single package: `figma-mcp-free`.

The repository stays a pnpm workspace because the internal boundaries help development, tests, and fork contributions. Publishing every internal workspace package would force first-time use to depend on scoped-package ownership, release ordering, and six synchronized versions. The release candidate instead embeds the five internal runtime packages inside one npm tarball while leaving ordinary third-party dependencies visible to npm.

## User-facing commands

After a verified npm release, one package exposes these entry points:
...
The package is not published merely because the tarball builds. Public install commands must be documented only after the registry package is independently verified.
...
Registry publication or a GitHub Release remains blocked until all of the following are true:
